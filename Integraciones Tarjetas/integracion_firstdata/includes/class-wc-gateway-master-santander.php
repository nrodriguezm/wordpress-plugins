...
/**
 * Pasarela Master Card Santander
 *
 */
class WC_Gateway_MasterSantander extends WC_Payment_Gateway {

	public function __construct() {
		$this->id 					= 'first_data_mastersantander';
		$this->campo_cuotas 		= 'installments-'.$this->id;
		$this->icon 				= get_site_url().'/wp-content/plugins/integracion_firstdata/img/master.png';
		$this->has_fields			= true;
		$this->method_title 		= 'Santander Master Card';
		$this->notify_url       	= add_query_arg( 'wc-api', 'WC_Gateway_MasterSantander', home_url( '/' ));
		$this->enabled 				= 'no';
		$this->bincc 				= '000001';
		...
		add_action( 'woocommerce_api_wc_gateway_mastersantander', array( $this, 'respuesta_firstdata' ) ); 
	}
	...
	function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled' => array(
							...
							'type' => 'checkbox',
							'label' => __( 'Habilitar Santander Master Card', 'woocommerce' ),
							'default' => 'no'
						),
			'title' => array(
							...
							'default' => __( 'Santander Master Card', 'woocommerce' ),
							'desc_tip'      => true,
						),
			'description' => array(
							...
							'default' => __( 'Puede pagar con una tarjeta de crédito Santander Master Card con total seguridad.', 'woocommerce' )
						),
			'fd_merchant' => array(
	...
	public function payment_fields(){
		...
		echo "<label style='margin-right:8px;'>Primeros 6 dígitos </label><input pattern='[0-9]' maxlength='6' type='text' name='num_tarjeta'><br><br>";
	...
	function process_payment( $order_id ) {
		...
		$order->update_status('processing', 'El usuario inició la compra con Santander Master Card.');
	...
	public function validate_fields(){
		...
		//Numeros permitidos de Santander MASTER
		$permitidos = array(
			"510198", 
			"519757", 
			"521119", 
		...
		if(!in_array($_POST['num_tarjeta'], $permitidos)){
			wc_add_notice("El número de tarjeta no es Santander", "error");
			return false;
		}
