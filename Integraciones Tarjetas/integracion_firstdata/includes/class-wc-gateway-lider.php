...
class WC_Gateway_Lider extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id 					= 'first_data_lider';
		$this->campo_cuotas 		= 'installments-'.$this->id;
		$this->icon 				= get_site_url().'/wp-content/plugins/integracion_firstdata/img/lider.png';
		$this->has_fields			= true;
		$this->method_title 		= 'Lider';
		$this->notify_url       	= add_query_arg( 'wc-api', 'WC_Gateway_Lider', home_url( '/' ));
		$this->enabled 				= 'no';
		$this->bincc 				= '000006';
		...
		add_action( 'woocommerce_api_wc_gateway_lider', array( $this, 'respuesta_firstdata' ) ); 
	}
	...
	function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'description' => array(
							...
							'default' => __( 'Puede pagar con una tarjeta de crédito Lider con total seguridad.', 'woocommerce' )
						),
			'fd_merchant' => array(
	...
	function process_payment( $order_id ) {
		...
		$order->update_status('processing', 'El usuario inició la compra con Lider.');
