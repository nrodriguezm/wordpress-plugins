 ...
 *
 */
class WC_Gateway_Discover extends WC_Payment_Gateway {

	public function __construct() {
		$this->id 					= 'first_data_discover';
		$this->campo_cuotas 		= 'installments-'.$this->id;
		$this->icon 				= get_site_url().'/wp-content/plugins/integracion_firstdata/img/discover.png';
		$this->has_fields			= true;
		$this->method_title 		= 'Discover';
		$this->notify_url       	= add_query_arg( 'wc-api', 'WC_Gateway_Discover', home_url( '/' ));
		$this->enabled 				= 'no';
		$this->bincc 				= '000005';
		...
		add_action( 'woocommerce_api_wc_gateway_discover', array( $this, 'respuesta_firstdata' ) ); 
	}
	...
	function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled' => array(
							...
							'type' => 'checkbox',
							'label' => __( 'Habilitar Discover', 'woocommerce' ),
							'default' => 'no'
						),
			'title' => array(
							...
							'default' => __( 'Discover', 'woocommerce' ),
							'desc_tip'      => true,
						),
			'description' => array(
							...
							'default' => __( 'Puede pagar con una tarjeta de crédito Discover con total seguridad.', 'woocommerce' )
						),
			'fd_merchant' => array(
	...
	function process_payment( $order_id ) {
		...
		$order->update_status('processing', 'El usuario inició la compra con Discover.');
