...
function add_to_spreadsheet_nd(){
	...
	//Get form id
	$id_form = $ninja_forms_processing->get_form_ID();
	...
	if( is_array( $fields ) and !is_null($form_values) ){ 
		...
		foreach($fields as $key => $value){
			$exist_key = array_key_exists("nd-field_$key", $google_values);
			if(!is_null($value) and !empty($value) and $exist_key){
				...
				$strRequest .= $google_values["nd-field_$key"]."=".urlencode($value);
			}
		}
...
add_action( 'ninja_forms_process', 'add_to_spreadsheet_nd' );
?>
